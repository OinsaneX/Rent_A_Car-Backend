...
emailCtrl.sendTestEmail = async (req, res) => {
  await nodemailer.createTestAccount(async (err, account) => {
    ...
    <a href="https://rent-a-car-zeta.vercel.app/profile/confirm/${req.body.id}">Para validar su cuenta haga click aqui</a>
    `;
  ...
  await nodemailer.createTestAccount(async (err, account) => {
    const htmlEmail = `
        <p>Email enviado automaticamente desde Rent_A_Car</p>
         ...
         <h3>Fecha de recogida : ${new Date(rent.pickUp).getDate()}/${
      new Date(rent.pickUp).getMonth() + 1
    }/${new Date(rent.pickUp).getFullYear()}</h3>
         <h3>Fecha de entrega : ${new Date(rent.dropOff).getDate()}/${
      new Date(rent.dropOff).getMonth() + 1
         ...
         <h3>Hora de recogida : ${
           rent.pickHour < 13 ? `${rent.pickHour} AM` : `${rent.pickHour} PM`
         }</h3>
         <h3>Hora de entrega : ${
           rent.dropHour < 13 ? `${rent.dropHour} AM` : `${rent.dropHour} PM`
         }</h3>

        <h3>Gracias por usar nuestro servicio .Para confirmar su reserva haga click <a href="https://rent-a-car-zeta.vercel.app/rent/confirm/${
          rent._id
        }">aqui</a></h3>
        `;
    let transporter = await nodemailer.createTransport({
...
emailCtrl.sendNotificationForm = async (req, res) => {
  ...
  await nodemailer.createTestAccount(async (err, account) => {
    const htmlEmail = `
            <p>Email enviado automaticamente desde Rent_A_Car</p>
             <h2>Se ha registrado una nueva petición de chofer en el sistema. Acceda con su cuenta comercial y responda lo antes posible </h2>
